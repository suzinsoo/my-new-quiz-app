...
const App = () => {
    ...
    const generateQuiz = async () => {
        ...
        try {
            ...
            const prompt = `사용자 "${quizCreatorName}"의 성격과 성향, 좋아하는 것, 싫어하는 것을 구분해서 1000자 내외로 분석한 내용입니다: "${personalityDescription}". 이 내용을 바탕으로 "${quizCreatorName}"를 맞춰보는 재미있고 흥미로운 객관식 퀴즈 질문 10개를 생성해 주세요. 각 질문은 4개의 보기(1, 2, 3, 4)를 포함해야 하며, **나머지 3개의 오답은 정답과 전혀 관련 없는, 동떨어진 내용으로 구성하여 변별력을 높여주세요.** 출력은 'question', 'options', 'compatibleAnswer' 필드를 가진 JSON 객체 배열이어야 합니다. 'compatibleAnswer'는 '1', '2', '3', '4' 중 하나여야 합니다.`;
